...
class TestSystemAPI:
    ...
    def test_update_dept_success(self):
        ...
        response = self.client.put(
            f"{self.base_url}/depts/{dept['dept_id']}",
    ...
    def test_update_menu_success(self):
        ...
        response = self.client.put(
            f"{self.base_url}/menus/{menu['menu_id']}",
            data=json.dumps(update_data),
            content_type="application/json",
        )
        assert response.status_code == 200
    ...
    def test_assign_menus_to_role_success(self):
        ...
        response = self.client.post(
            f"{self.base_url}/roles/{role['role_id']}/menus",
    ...
    def test_list_operation_logs_with_filter(self):
        ...
        response = self.client.get(
            f"{self.base_url}/operation-logs?module=auth&method=POST&page=1&page_size=10"
        )
        assert response.status_code == 200
        data = json.loads(response.content)
        assert "logs" in data
