...
class SecurityController:
    ...
    @http_post("/security/blacklist", response=IPBlacklistResponseDTO, summary="添加IP到黑名单", operation_id="security_add_blacklist")
    async def add_to_blacklist(self, blacklist_dto: IPBlacklistDTO) -> IPBlacklistResponseDTO:
        """
        添加IP到黑名单
    ...
    @http_delete("/security/blacklist/{ip_address}", response=MessageResponse, summary="从黑名单移除IP", operation_id="security_remove_blacklist")
    ...
    @http_get("/security/blacklist", response=list[IPBlacklistResponseDTO], summary="获取黑名单列表", operation_id="security_list_blacklist")
    ...
    @http_post("/security/whitelist", response=IPWhitelistResponseDTO, summary="添加IP到白名单", operation_id="security_add_whitelist")
    async def add_to_whitelist(self, whitelist_dto: IPWhitelistDTO) -> IPWhitelistResponseDTO:
        """
        添加IP到白名单
    ...
    @http_delete("/security/whitelist/{ip_address}", response=MessageResponse, summary="从白名单移除IP", operation_id="security_remove_whitelist")
    ...
    @http_get("/security/whitelist", response=list[IPWhitelistResponseDTO], summary="获取白名单列表", operation_id="security_list_whitelist")
    ...
    @http_post("/security/rate-limit", response=RateLimitRuleResponseDTO, summary="创建限流规则", operation_id="security_create_rate_limit")
    ...
    @http_delete("/security/rate-limit/{rule_id}", response=MessageResponse, summary="删除限流规则", operation_id="security_delete_rate_limit")
    ...
    @http_get("/security/rate-limit", response=list[RateLimitRuleResponseDTO], summary="获取限流规则列表", operation_id="security_list_rate_limits")
    ...
    @http_get("/security/status", summary="获取安全状态", operation_id="security_get_status")
