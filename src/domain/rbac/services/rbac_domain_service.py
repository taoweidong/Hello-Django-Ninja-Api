...
class RBACDomainService:
    ...
    async def create_role(self, name: str, code: str, description: str = "", permissions: list[str] = None, created_by: str = None) -> RoleEntity:
        """创建角色"""
        # 检查角色代码是否已存在
        ...
        role = RoleEntity(name=name, code=code, description=description, permissions=permissions or [], created_by=created_by)
    ...
    async def create_permission(self, name: str, code: str, description: str = "") -> PermissionEntity:
        """创建权限"""
        existing = await self.rbac_repository.get_permission_by_code(code)
        if existing:
            raise ValueError(f"权限代码 {code} 已存在")

        permission = PermissionEntity(name=name, code=code, description=description)
