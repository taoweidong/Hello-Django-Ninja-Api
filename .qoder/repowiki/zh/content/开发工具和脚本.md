# 开发工具和脚本

<cite>
**本文引用的文件**
- [pyproject.toml](file://pyproject.toml)
- [.mypy.ini](file://.mypy.ini)
- [ruff.toml](file://ruff.toml)
- [requirements.txt](file://requirements.txt)
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/migrate.sh](file://scripts/migrate.sh)
- [scripts/check_and_fix.py](file://scripts/check_and_fix.py)
- [scripts/simple_check.py](file://scripts/simple_check.py)
- [scripts/migrate_database.py](file://scripts/migrate_database.py)
- [scripts/integration_test.py](file://scripts/integration_test.py)
- [scripts/deploy_and_test.py](file://scripts/deploy_and_test.py)
- [scripts/init_admin.py](file://scripts/init_admin.py)
- [scripts/run_all.bat](file://scripts/run_all.bat)
- [scripts/setup_dev.bat](file://scripts/setup_dev.bat)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向 Hello-Django-Ninja-Api 项目的开发者，系统介绍项目中的开发工具链与自动化脚本，包括：
- 代码格式化与检查：Ruff（格式化、lint、导入排序、命名规范、性能与简化建议等）
- 类型检查：MyPy（含 Django 插件与忽略规则）
- 测试与覆盖率：pytest（含覆盖率统计）
- 数据库迁移与初始化：Django 命令与自定义迁移脚本
- 自动化脚本：Linux/macOS 与 Windows 双平台脚本
- IDE 配置建议与开发工作流优化
- 代码质量保障流程与静态分析工具集成
- 构建系统与依赖管理策略
- 提高开发效率的实用工具推荐

## 项目结构
项目采用分层架构与功能模块划分，开发工具与脚本集中在 scripts 目录，配置文件位于仓库根目录。关键配置文件如下：
- pyproject.toml：项目元数据、依赖、构建系统、pytest、Ruff、MyPy、django-stubs、覆盖率等配置
- .mypy.ini：MyPy 的独立配置文件（兼容多处配置）
- ruff.toml：Ruff 的统一配置文件（格式化、lint、导入排序）
- requirements.txt：生产依赖清单
- scripts/*：各类开发脚本（Linux/macOS 与 Windows）

```mermaid
graph TB
subgraph "配置与依赖"
P["pyproject.toml"]
M[".mypy.ini"]
R["ruff.toml"]
REQ["requirements.txt"]
end
subgraph "脚本"
LINT["scripts/lint.sh"]
TEST["scripts/test.sh"]
SETUP["scripts/setup_dev.sh"]
MIG["scripts/migrate.sh"]
CF["scripts/check_and_fix.py"]
SC["scripts/simple_check.py"]
MD["scripts/migrate_database.py"]
IT["scripts/integration_test.py"]
DT["scripts/deploy_and_test.py"]
IA["scripts/init_admin.py"]
RUNALL["scripts/run_all.bat"]
SETUP_WIN["scripts/setup_dev.bat"]
end
P --> LINT
P --> TEST
P --> SETUP
P --> MIG
P --> CF
P --> SC
P --> MD
P --> IT
P --> DT
P --> IA
P --> RUNALL
P --> SETUP_WIN
R --> LINT
R --> SETUP
R --> CF
R --> SC
M --> TEST
M --> SETUP
M --> CF
M --> SC
```

图表来源
- [pyproject.toml](file://pyproject.toml)
- [ruff.toml](file://ruff.toml)
- [.mypy.ini](file://.mypy.ini)
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/migrate.sh](file://scripts/migrate.sh)
- [scripts/check_and_fix.py](file://scripts/check_and_fix.py)
- [scripts/simple_check.py](file://scripts/simple_check.py)
- [scripts/migrate_database.py](file://scripts/migrate_database.py)
- [scripts/integration_test.py](file://scripts/integration_test.py)
- [scripts/deploy_and_test.py](file://scripts/deploy_and_test.py)
- [scripts/init_admin.py](file://scripts/init_admin.py)
- [scripts/run_all.bat](file://scripts/run_all.bat)
- [scripts/setup_dev.bat](file://scripts/setup_dev.bat)

章节来源
- [pyproject.toml](file://pyproject.toml)
- [ruff.toml](file://ruff.toml)
- [.mypy.ini](file://.mypy.ini)
- [requirements.txt](file://requirements.txt)

## 核心组件
- 代码格式化与检查（Ruff）
  - 统一配置：ruff.toml 与 pyproject.toml 中的 [tool.ruff] 部分
  - 规则集：启用 pycodestyle、pyflakes、isort、pep8-naming、flake8-* 等
  - 特殊忽略：针对 migrations、tests、config 等目录的特定规则
  - 格式化：双引号、空格缩进、换行策略
- 类型检查（MyPy）
  - 配置：.mypy.ini 与 pyproject.toml 中的 [tool.mypy] 与 plugins
  - Django 支持：django-stubs 插件，settings_module 指向 config.settings
  - 忽略策略：忽略缺失导入、测试与 migrations 目录的类型错误
- 测试与覆盖率（pytest）
  - 配置：pyproject.toml 中的 [tool.pytest.ini_options]
  - 覆盖率：覆盖 src，排除 migrations/tests/config/manage.py
- 数据库迁移与初始化
  - Django 命令：makemigrations、migrate、createsuperuser
  - 自定义脚本：迁移数据库文件位置、创建超级管理员、清理缓存

章节来源
- [pyproject.toml](file://pyproject.toml)
- [.mypy.ini](file://.mypy.ini)
- [ruff.toml](file://ruff.toml)

## 架构总览
下图展示开发工具链与脚本之间的关系，以及它们如何协同完成代码质量保障与自动化任务。

```mermaid
graph TB
DEV["开发者"]
VENV["Python 虚拟环境"]
RUFF["Ruff<br/>格式化/检查/导入排序"]
MYPY["MyPy<br/>类型检查(Django 插件)"]
PYTEST["pytest<br/>测试/覆盖率"]
DJANGO["Django 管理命令<br/>makemigrations/migrate/createsuperuser"]
SCRIPTS["scripts/*<br/>脚本集合"]
DEV --> VENV
VENV --> RUFF
VENV --> MYPY
VENV --> PYTEST
VENV --> DJANGO
VENV --> SCRIPTS
SCRIPTS --> RUFF
SCRIPTS --> MYPY
SCRIPTS --> PYTEST
SCRIPTS --> DJANGO
```

图表来源
- [pyproject.toml](file://pyproject.toml)
- [ruff.toml](file://ruff.toml)
- [.mypy.ini](file://.mypy.ini)
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/migrate.sh](file://scripts/migrate.sh)

## 详细组件分析

### Ruff 配置与使用
- 配置要点
  - 规则集：E/W/F/I/N/UP/B/C4/SIM/ARG/PTH/PERF 等
  - 忽略规则：行长度、函数调用作为默认参数、复杂度、裸异常等
  - per-file-ignores：对 __init__.py、migrations、tests、config 的特殊处理
  - 导入排序：known-first-party 与 known-third-party 分组
  - 格式化：双引号、空格缩进、自动换行
- 在脚本中的应用
  - lint.sh：先格式化检查，再代码检查，最后类型检查
  - setup_dev.sh：创建虚拟环境后，统一执行格式化与检查
  - check_and_fix.py / simple_check.py：跨平台调用 ruff check/format 并输出结果
- 最佳实践
  - 在提交前运行 ruff format --check 与 ruff check
  - 避免在 CI 中强制 fix，保持本地修复、CI 仅检查

```mermaid
flowchart TD
START(["开始"]) --> ACTIVATE["激活虚拟环境"]
ACTIVATE --> FORMAT["Ruff 格式化检查"]
FORMAT --> CHECK["Ruff 代码检查"]
CHECK --> MYPY["MyPy 类型检查"]
MYPY --> DONE(["结束"])
```

图表来源
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/check_and_fix.py](file://scripts/check_and_fix.py)
- [scripts/simple_check.py](file://scripts/simple_check.py)

章节来源
- [ruff.toml](file://ruff.toml)
- [pyproject.toml](file://pyproject.toml)
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/check_and_fix.py](file://scripts/check_and_fix.py)
- [scripts/simple_check.py](file://scripts/simple_check.py)

### MyPy 配置与使用
- 配置要点
  - Python 版本：3.10
  - 关键开关：严格可选、冗余转换、未使用忽略、显式包基础等
  - Django 插件：mypy_django_plugin.main，settings_module 指向 config.settings
  - 忽略策略：忽略缺失导入、测试与 migrations 目录
- 在脚本中的应用
  - lint.sh：在 Ruff 检查后执行 mypy src/
  - setup_dev.sh：安装 dev 依赖后执行 mypy src/
  - check_and_fix.py / simple_check.py：调用 mypy 对 src/ 进行类型检查
- 最佳实践
  - 逐步收紧 disallow_untyped_defs 等规则
  - 为第三方库添加 stub 或忽略缺失导入

```mermaid
flowchart TD
A["加载 MyPy 配置"] --> B["扫描 src/ 模块"]
B --> C{"发现未注解/缺失导入?"}
C --> |是| D["输出告警/错误"]
C --> |否| E["类型检查通过"]
D --> F["根据规则调整代码或忽略"]
E --> G["完成"]
```

图表来源
- [.mypy.ini](file://.mypy.ini)
- [pyproject.toml](file://pyproject.toml)
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/check_and_fix.py](file://scripts/check_and_fix.py)
- [scripts/simple_check.py](file://scripts/simple_check.py)

章节来源
- [.mypy.ini](file://.mypy.ini)
- [pyproject.toml](file://pyproject.toml)

### 测试与覆盖率（pytest）
- 配置要点
  - DJANGO_SETTINGS_MODULE：指向 testing 配置
  - 测试文件模式：test_*.py、*_test.py
  - 断言与标记：严格标记与配置、短回溯
  - 覆盖率：覆盖 src，排除 migrations/tests/config/manage.py
- 在脚本中的应用
  - test.sh：运行 pytest 并生成 HTML 与终端覆盖率报告
  - setup_dev.sh：安装 dev 依赖后运行 pytest
  - integration_test.py：对各 API 进行端到端验证，生成结果文件
- 最佳实践
  - 为不同测试类型添加标记（unit/integration/slow）
  - 结合覆盖率报告定位未覆盖区域

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant Pytest as "pytest"
participant Cov as "覆盖率"
participant Report as "HTML/终端报告"
Dev->>Pytest : 运行测试
Pytest->>Cov : 收集覆盖率
Cov-->>Report : 生成报告
Pytest-->>Dev : 输出测试结果
```

图表来源
- [pyproject.toml](file://pyproject.toml)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/integration_test.py](file://scripts/integration_test.py)

章节来源
- [pyproject.toml](file://pyproject.toml)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/integration_test.py](file://scripts/integration_test.py)

### 数据库迁移与初始化
- Django 命令
  - makemigrations：生成迁移文件
  - migrate：应用迁移
  - createsuperuser：创建超级管理员（支持非交互）
- 自定义脚本
  - migrate.sh：封装迁移与管理员创建
  - migrate_database.py：迁移数据库文件位置、创建迁移、应用迁移、创建管理员、清理缓存
  - init_admin.py：通过 Django shell 创建初始管理员，避免导入链问题
- 最佳实践
  - 在 CI 中使用 --run-syncdb 确保同步数据库
  - 使用环境变量控制管理员用户名/邮箱/密码

```mermaid
flowchart TD
S["开始"] --> MK["生成迁移文件"]
MK --> AP["应用迁移"]
AP --> SU["创建超级管理员"]
SU --> CLEAN["清理缓存与临时文件"]
CLEAN --> END["完成"]
```

图表来源
- [scripts/migrate.sh](file://scripts/migrate.sh)
- [scripts/migrate_database.py](file://scripts/migrate_database.py)
- [scripts/init_admin.py](file://scripts/init_admin.py)

章节来源
- [scripts/migrate.sh](file://scripts/migrate.sh)
- [scripts/migrate_database.py](file://scripts/migrate_database.py)
- [scripts/init_admin.py](file://scripts/init_admin.py)

### 自动化脚本总览
- Linux/macOS
  - lint.sh：格式化检查 → 代码检查 → 类型检查
  - test.sh：运行 pytest，生成覆盖率报告
  - setup_dev.sh：安装 uv、创建虚拟环境、安装 dev 依赖、格式化/检查/类型检查、创建管理员、运行测试
- Windows
  - run_all.bat：顺序执行迁移、创建管理员、启动服务、API 测试
  - setup_dev.bat：安装 uv、创建虚拟环境、安装 dev 依赖、格式化/检查/类型检查、创建管理员、运行测试

```mermaid
graph LR
subgraph "Linux/macOS"
L["lint.sh"]
T["test.sh"]
SD["setup_dev.sh"]
end
subgraph "Windows"
RA["run_all.bat"]
SDW["setup_dev.bat"]
end
L --> T
SD --> T
RA --> SDW
```

图表来源
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/run_all.bat](file://scripts/run_all.bat)
- [scripts/setup_dev.bat](file://scripts/setup_dev.bat)

章节来源
- [scripts/lint.sh](file://scripts/lint.sh)
- [scripts/test.sh](file://scripts/test.sh)
- [scripts/setup_dev.sh](file://scripts/setup_dev.sh)
- [scripts/run_all.bat](file://scripts/run_all.bat)
- [scripts/setup_dev.bat](file://scripts/setup_dev.bat)

### 集成测试脚本
- 功能概述
  - 使用 requests 对每个接口进行真实测试
  - 包含健康检查、认证流程、用户管理、RBAC、安全管理、系统管理等
  - 记录测试结果并生成汇总文件
- 使用方式
  - 直接运行脚本，确保服务已启动
  - 可结合 pytest 与覆盖率进行综合质量评估

```mermaid
sequenceDiagram
participant IT as "集成测试"
participant API as "Django API"
participant AUTH as "认证"
participant USER as "用户管理"
participant RBAC as "RBAC"
participant SEC as "安全管理"
participant SYS as "系统管理"
IT->>API : GET /api/health
IT->>AUTH : POST /v1/auth/login
IT->>USER : GET /v1/users
IT->>RBAC : POST /v1/rbac/roles
IT->>SEC : GET /v1/security/blacklist
IT->>SYS : GET /v1/system/health
IT-->>IT : 汇总结果并输出
```

图表来源
- [scripts/integration_test.py](file://scripts/integration_test.py)

章节来源
- [scripts/integration_test.py](file://scripts/integration_test.py)

### 部署与测试流水线
- deploy_and_test.py
  - 顺序执行：创建迁移 → 应用迁移 → 创建管理员 → 启动服务 → API 功能测试 → 单元测试
  - 启动服务后进行健康检查
  - 输出服务信息与提示

```mermaid
flowchart TD
D0["开始"] --> D1["创建迁移文件"]
D1 --> D2["应用迁移"]
D2 --> D3["创建管理员"]
D3 --> D4["启动服务"]
D4 --> D5["健康检查"]
D5 --> D6["API 功能测试"]
D6 --> D7["单元测试"]
D7 --> D8["完成"]
```

图表来源
- [scripts/deploy_and_test.py](file://scripts/deploy_and_test.py)

章节来源
- [scripts/deploy_and_test.py](file://scripts/deploy_and_test.py)

## 依赖分析
- 依赖管理策略
  - 生产依赖：requirements.txt（集中列出）
  - 开发依赖：pyproject.toml 中的 optional-dependencies.dev（pytest、ruff、mypy、django-stubs、faker 等）
  - 构建系统：hatchling
- 工具链耦合
  - Ruff 与 MyPy 配置相互独立但共同作用于 src/ 与 tests/
  - pytest 与覆盖率配置集中在 pyproject.toml
  - Django 与 django-stubs 插件配合 MyPy 实现类型感知

```mermaid
graph TB
REQ["requirements.txt<br/>生产依赖"] --> APP["应用"]
DEV["pyproject.toml<br/>dev 依赖"] --> TOOL["开发工具链"]
TOOL --> RUFF["Ruff"]
TOOL --> MYPY["MyPy"]
TOOL --> PYTEST["pytest"]
APP --> DJ["Django"]
DJ --> STUBS["django-stubs"]
MYPY --> STUBS
```

图表来源
- [requirements.txt](file://requirements.txt)
- [pyproject.toml](file://pyproject.toml)

章节来源
- [requirements.txt](file://requirements.txt)
- [pyproject.toml](file://pyproject.toml)

## 性能考虑
- 代码检查性能
  - 使用缓存目录（如 .ruff_cache、.mypy_cache）提升重复检查速度
  - 在 CI 中避免 --fix，减少格式化开销
- 测试性能
  - 使用 pytest 标记区分测试类型，按需运行
  - 覆盖率仅覆盖 src，缩短报告生成时间
- 迁移性能
  - 在本地开发中使用 SQLite；在 CI 中使用 PostgreSQL 以更接近生产环境

## 故障排查指南
- Ruff 相关
  - 格式化失败：优先在本地运行 ruff format --check，修正后再提交
  - 导入排序冲突：检查 isort 配置与 known-* 分组
  - per-file-ignores 生效：确认文件路径匹配（如 migrations、tests、config）
- MyPy 相关
  - 缺失导入：在 .mypy.ini 中添加 ignore_missing_imports 或安装 stub
  - Django 类型错误：确认 django-stubs 插件已启用且 settings_module 正确
  - 测试与 migrations 忽略：检查对应段落配置
- 测试相关
  - pytest 失败：查看短回溯输出，结合覆盖率定位问题
  - 覆盖率不准确：检查 omit 列表与 source 配置
- 迁移相关
  - 数据库迁移失败：查看 Django 报错信息，必要时回滚并重试
  - 管理员创建失败：检查环境变量与用户是否存在

章节来源
- [ruff.toml](file://ruff.toml)
- [.mypy.ini](file://.mypy.ini)
- [pyproject.toml](file://pyproject.toml)

## 结论
本指南系统梳理了 Hello-Django-Ninja-Api 项目的开发工具链与脚本，涵盖 Ruff、MyPy、pytest、Django 迁移与初始化等关键环节。通过统一的配置文件与自动化脚本，开发者可以高效地完成代码质量保障、测试与部署流程。建议在团队内推广使用这些脚本与配置，持续改进代码质量与开发效率。

## 附录
- IDE 配置建议
  - VSCode：安装 Python、Ruff、MyPy、Django 插件；配置 Python 解释器为虚拟环境
  - PyCharm：启用 Ruff 与 MyPy 集成，配置 Django 项目结构与 settings_module
- 开发工作流优化
  - 提交前：ruff format --check + ruff check + mypy src/ + pytest
  - CI：仅运行检查与测试，不执行 --fix
  - 定期更新依赖：使用 uv pip install -e ".[dev]" 更新开发依赖
- 实用工具推荐
  - uv：快速安装与虚拟环境管理
  - httpie/curl：API 接口调试
  - pytest-watch：测试热重载