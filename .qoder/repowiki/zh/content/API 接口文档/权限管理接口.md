# 权限管理接口

<cite>
**本文引用的文件**
- [rbac_controller.py](file://src/api/v1/controllers/rbac_controller.py)
- [app.py](file://src/api/app.py)
- [rbac_service.py](file://src/application/services/rbac_service.py)
- [rbac_domain_service.py](file://src/domain/rbac/services/rbac_domain_service.py)
- [rbac_repo_impl.py](file://src/infrastructure/repositories/rbac_repo_impl.py)
- [rbac_models.py](file://src/infrastructure/persistence/models/rbac_models.py)
- [role_create_dto.py](file://src/application/dto/rbac/role_create_dto.py)
- [role_update_dto.py](file://src/application/dto/rbac/role_update_dto.py)
- [assign_role_dto.py](file://src/application/dto/rbac/assign_role_dto.py)
- [user_roles_response_dto.py](file://src/application/dto/rbac/user_roles_response_dto.py)
- [role_response_dto.py](file://src/application/dto/rbac/role_response_dto.py)
- [permission_response_dto.py](file://src/application/dto/rbac/permission_response_dto.py)
- [role_entity.py](file://src/domain/rbac/entities/role_entity.py)
- [permission_entity.py](file://src/domain/rbac/entities/permission_entity.py)
- [cache_manager.py](file://src/infrastructure/cache/cache_manager.py)
- [responses.py](file://src/api/common/responses.py)
- [permissions.py](file://src/api/common/permissions.py)
- [security_middleware.py](file://src/core/middlewares/security_middleware.py)
- [test_rbac_api.py](file://tests/test_api/test_rbac_api.py)
- [rbac.sql](file://sql/rbac.sql)
</cite>

## 更新摘要
**变更内容**
- RBAC系统从独立API文件迁移到控制器架构
- 使用NinjaExtra的@api_controller装饰器重构API接口
- 改进DTO处理和错误管理机制
- API注册方式从函数式路由改为控制器注册模式
- 新增统一的响应格式和错误处理

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件为 RBAC 权限管理接口的专业 API 文档，覆盖角色管理、权限分配、用户角色绑定等核心权限功能。内容包括：
- 角色 CRUD 操作、权限查询、用户角色列表等接口的完整规范
- RBAC 架构设计原理、角色层级关系与权限继承机制
- 权限检查流程、访问控制决策与权限缓存策略的技术实现
- 权限模型的数据结构、关系映射与约束规则
- 批量权限操作、权限树形结构与动态权限验证的实现细节
- 权限管理的最佳实践与安全考虑

**更新** RBAC系统现已迁移到控制器架构，提供更清晰的代码组织和更好的错误管理能力。

## 项目结构
RBAC 子系统采用分层架构：控制器层负责HTTP请求处理与响应格式化；应用服务层承载业务逻辑；领域服务与仓储层分别抽象业务规则与数据持久化；模型层定义数据库表结构；缓存层提供权限与角色缓存能力。

```mermaid
graph TB
subgraph "控制器层"
C1["rbac_controller.py<br/>RBACController<br/>@api_controller装饰器"]
end
subgraph "应用服务层"
S1["rbac_service.py<br/>RBACService"]
end
subgraph "领域层"
D1["rbac_domain_service.py<br/>RBACDomainService"]
E1["role_entity.py<br/>RoleEntity"]
E2["permission_entity.py<br/>PermissionEntity"]
end
subgraph "仓储层"
R2["rbac_repo_impl.py<br/>RBACRepositoryImpl"]
end
subgraph "数据模型层"
M1["rbac_models.py<br/>Permission/Role/UserRole"]
end
subgraph "缓存层"
CM["cache_manager.py<br/>CacheManager"]
end
C1 --> S1
S1 --> R2
D1 --> R2
R2 --> M1
S1 --> CM
```

**图表来源**
- [rbac_controller.py:31-49](file://src/api/v1/controllers/rbac_controller.py#L31-L49)
- [rbac_service.py:15-23](file://src/application/services/rbac_service.py#L15-L23)
- [rbac_domain_service.py:1-144](file://src/domain/rbac/services/rbac_domain_service.py#L1-L144)
- [rbac_repo_impl.py:1-251](file://src/infrastructure/repositories/rbac_repo_impl.py#L1-L251)
- [rbac_models.py:1-148](file://src/infrastructure/persistence/models/rbac_models.py#L1-L148)
- [cache_manager.py:1-149](file://src/infrastructure/cache/cache_manager.py#L1-L149)

**章节来源**
- [rbac_controller.py:1-259](file://src/api/v1/controllers/rbac_controller.py#L1-L259)
- [rbac_service.py:1-280](file://src/application/services/rbac_service.py#L1-L280)
- [rbac_domain_service.py:1-144](file://src/domain/rbac/services/rbac_domain_service.py#L1-L144)
- [rbac_repo_impl.py:1-251](file://src/infrastructure/repositories/rbac_repo_impl.py#L1-L251)
- [rbac_models.py:1-148](file://src/infrastructure/persistence/models/rbac_models.py#L1-L148)
- [cache_manager.py:1-149](file://src/infrastructure/cache/cache_manager.py#L1-L149)

## 核心组件
- **控制器层**
  - RBACController：使用NinjaExtra的@api_controller装饰器，提供角色管理、权限管理、用户角色关联与权限检查接口
  - 支持依赖注入，可通过构造函数注入RBACService实例
  - 提供统一的响应格式和错误处理机制
- **应用服务层**
  - RBACService：封装角色 CRUD、权限初始化、用户角色分配与权限检查等业务逻辑，并集成缓存管理
- **领域服务与实体**
  - RBACDomainService：以领域驱动方式封装角色与权限的业务规则
  - RoleEntity、PermissionEntity：定义角色与权限的领域实体，包含权限代码解析、激活/停用等行为
- **仓储层**
  - RBACRepositoryImpl：实现角色、权限与用户角色关联的数据库操作，支持权限集合计算与权限校验
- **数据模型**
  - Permission、Role、UserRole：Django ORM 模型，定义权限、角色与用户角色关联的表结构与索引
- **缓存管理**
  - CacheManager：提供用户权限与角色缓存的统一接口，支持读取、写入与失效

**章节来源**
- [rbac_controller.py:31-49](file://src/api/v1/controllers/rbac_controller.py#L31-L49)
- [rbac_controller.py:53-259](file://src/api/v1/controllers/rbac_controller.py#L53-L259)
- [rbac_service.py:15-280](file://src/application/services/rbac_service.py#L15-L280)
- [rbac_domain_service.py:1-144](file://src/domain/rbac/services/rbac_domain_service.py#L1-L144)
- [rbac_repo_impl.py:1-251](file://src/infrastructure/repositories/rbac_repo_impl.py#L1-L251)
- [rbac_models.py:1-148](file://src/infrastructure/persistence/models/rbac_models.py#L1-L148)
- [cache_manager.py:1-149](file://src/infrastructure/cache/cache_manager.py#L1-L149)

## 架构总览
RBAC 接口遵循分层解耦与单一职责原则，请求从控制器层进入，经应用服务层处理业务逻辑，仓储层完成数据持久化，缓存层优化权限查询性能。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Controller as "RBACController"
participant Service as "RBACService"
participant Repo as "RBACRepositoryImpl"
participant DB as "Django ORM 模型"
Client->>Controller : "POST /v1/rbac/roles"
Controller->>Service : "create_role(RoleCreateDTO)"
Service->>Repo : "保存角色并建立权限关联"
Repo->>DB : "asave()/aset()"
DB-->>Repo : "持久化结果"
Repo-->>Service : "RoleEntity/PermissionEntity"
Service-->>Controller : "RoleResponseDTO"
Controller-->>Client : "200 OK + 角色信息"
```

**图表来源**
- [rbac_controller.py:53-70](file://src/api/v1/controllers/rbac_controller.py#L53-L70)
- [rbac_service.py:26-42](file://src/application/services/rbac_service.py#L26-L42)
- [rbac_repo_impl.py:34-41](file://src/infrastructure/repositories/rbac_repo_impl.py#L34-L41)
- [rbac_models.py:43-77](file://src/infrastructure/persistence/models/rbac_models.py#L43-L77)

## 详细组件分析

### 角色管理接口
- **创建角色**
  - 路径：POST /v1/rbac/roles
  - 输入：RoleCreateDTO（名称、代码、描述、权限代码列表）
  - 行为：校验角色代码唯一性，创建角色并建立权限关联
  - 输出：RoleResponseDTO
- **获取角色详情**
  - 路径：GET /v1/rbac/roles/{role_id}
  - 行为：按 ID 查询角色，不存在时抛出ValueError异常
- **获取角色列表**
  - 路径：GET /v1/rbac/roles
  - 参数：is_active（可选）
  - 行为：按激活状态过滤角色，返回RoleListResponse格式
- **更新角色**
  - 路径：PUT /v1/rbac/roles/{role_id}
  - 输入：RoleUpdateDTO（名称、描述、权限代码列表）
  - 行为：系统角色不可修改，更新后重建权限关联
- **删除角色**
  - 路径：DELETE /v1/rbac/roles/{role_id}
  - 行为：系统角色不可删除，删除成功后返回MessageResponse

```mermaid
flowchart TD
Start(["开始"]) --> Validate["校验输入参数"]
Validate --> Exists{"角色代码已存在？"}
Exists --> |是| Error["抛出ValueError异常"]
Exists --> |否| Create["创建角色记录"]
Create --> LinkPerms["建立权限关联"]
LinkPerms --> Success["返回RoleResponseDTO"]
Error --> End(["结束"])
Success --> End
```

**图表来源**
- [rbac_controller.py:53-70](file://src/api/v1/controllers/rbac_controller.py#L53-L70)
- [rbac_service.py:26-42](file://src/application/services/rbac_service.py#L26-L42)
- [rbac_repo_impl.py:34-41](file://src/infrastructure/repositories/rbac_repo_impl.py#L34-L41)

**章节来源**
- [rbac_controller.py:53-147](file://src/api/v1/controllers/rbac_controller.py#L53-L147)
- [rbac_service.py:26-92](file://src/application/services/rbac_service.py#L26-L92)
- [rbac_repo_impl.py:34-41](file://src/infrastructure/repositories/rbac_repo_impl.py#L34-L41)

### 权限管理接口
- **获取权限列表**
  - 路径：GET /v1/rbac/permissions
  - 参数：is_active、resource（可选）
  - 行为：按激活状态与资源类型过滤权限，返回PermissionListResponse格式
- **初始化系统权限**
  - 路径：POST /v1/rbac/permissions/init
  - 行为：基于预定义系统权限创建缺失的权限条目，返回MessageResponse

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Controller as "RBACController"
participant Service as "RBACService"
participant Repo as "RBACRepositoryImpl"
Client->>Controller : "GET /v1/rbac/permissions?is_active=&resource="
Controller->>Service : "list_permissions(is_active, resource)"
Service->>Repo : "查询权限列表"
Repo-->>Service : "PermissionEntity 列表"
Service-->>Controller : "PermissionResponseDTO 列表"
Controller-->>Client : "200 OK + 权限列表"
```

**图表来源**
- [rbac_controller.py:150-167](file://src/api/v1/controllers/rbac_controller.py#L150-L167)
- [rbac_service.py:122-138](file://src/application/services/rbac_service.py#L122-L138)
- [rbac_repo_impl.py:172-183](file://src/infrastructure/repositories/rbac_repo_impl.py#L172-L183)

**章节来源**
- [rbac_controller.py:150-180](file://src/api/v1/controllers/rbac_controller.py#L150-L180)
- [rbac_service.py:122-138](file://src/application/services/rbac_service.py#L122-L138)
- [rbac_repo_impl.py:172-183](file://src/infrastructure/repositories/rbac_repo_impl.py#L172-L183)

### 用户角色关联接口
- **分配角色给用户**
  - 路径：POST /v1/rbac/users/{user_id}/roles
  - 输入：AssignRoleDTO（user_id、role_id）
  - 行为：校验用户与角色存在性及激活状态，避免重复分配，分配后清理用户权限与角色缓存，返回MessageResponse
- **从用户移除角色**
  - 路径：DELETE /v1/rbac/users/{user_id}/roles/{role_id}
  - 行为：移除用户角色关联，清理缓存，不存在角色时抛出ValueError异常
- **获取用户角色与权限**
  - 路径：GET /v1/rbac/users/{user_id}/roles
  - 行为：返回UserRolesResponseDTO，包含用户角色列表与权限代码列表
- **检查用户权限**
  - 路径：GET /v1/rbac/users/{user_id}/permissions/check?permission_code=...
  - 行为：返回包含user_id、permission_code、has_permission的字典

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Controller as "RBACController"
participant Service as "RBACService"
participant Repo as "RBACRepositoryImpl"
participant Cache as "CacheManager"
Client->>Controller : "POST /v1/rbac/users/{user_id}/roles"
Controller->>Service : "assign_role_to_user(AssignRoleDTO)"
Service->>Repo : "校验用户/角色并创建关联"
Repo-->>Service : "成功"
Service->>Cache : "delete_permissions_cache(user_id)"
Service->>Cache : "delete_roles_cache(user_id)"
Controller-->>Client : "200 OK + MessageResponse"
```

**图表来源**
- [rbac_controller.py:184-203](file://src/api/v1/controllers/rbac_controller.py#L184-L203)
- [rbac_service.py:142-170](file://src/application/services/rbac_service.py#L142-L170)
- [rbac_repo_impl.py:186-194](file://src/infrastructure/repositories/rbac_repo_impl.py#L186-L194)
- [cache_manager.py:119-137](file://src/infrastructure/cache/cache_manager.py#L119-L137)

**章节来源**
- [rbac_controller.py:184-259](file://src/api/v1/controllers/rbac_controller.py#L184-L259)
- [rbac_service.py:142-212](file://src/application/services/rbac_service.py#L142-L212)
- [rbac_repo_impl.py:186-247](file://src/infrastructure/repositories/rbac_repo_impl.py#L186-L247)
- [cache_manager.py:108-137](file://src/infrastructure/cache/cache_manager.py#L108-L137)

### 权限检查流程与访问控制决策
- **中间件与权限类**
  - SecurityMiddleware：生产环境添加安全响应头
  - IsAuthenticated：校验 Bearer Token 并注入用户信息
  - HasPermission/HasAnyPermission：异步检查用户是否拥有指定权限
- **决策流程**
  - 首先执行认证，再执行权限异步检查，最终决定是否放行

```mermaid
flowchart TD
Req["收到请求"] --> Auth["IsAuthenticated 校验 Token"]
Auth --> |未认证| Deny["拒绝访问"]
Auth --> |已认证| Perm["HasPermission/HasAnyPermission 异步检查"]
Perm --> |无权限| Deny
Perm --> |有权限| Allow["放行"]
```

**图表来源**
- [security_middleware.py:33-53](file://src/core/middlewares/security_middleware.py#L33-L53)
- [permissions.py:14-121](file://src/api/common/permissions.py#L14-L121)
- [permissions.py:123-195](file://src/api/common/permissions.py#L123-L195)

**章节来源**
- [security_middleware.py:14-54](file://src/core/middlewares/security_middleware.py#L14-L54)
- [permissions.py:14-245](file://src/api/common/permissions.py#L14-L245)

### 权限缓存策略
- **缓存键命名**
  - 前缀：hello_api，分组：rbac
  - 用户权限缓存键：permissions:{user_id}
  - 用户角色缓存键：roles:{user_id}
- **缓存生命周期**
  - 权限缓存：默认 600 秒
  - 角色缓存：默认 600 秒
- **触发失效**
  - 分配/移除角色后主动删除对应用户权限与角色缓存，保证一致性

**章节来源**
- [cache_manager.py:16-149](file://src/infrastructure/cache/cache_manager.py#L16-L149)
- [rbac_service.py:166-169](file://src/application/services/rbac_service.py#L166-L169)
- [rbac_service.py:177-180](file://src/application/services/rbac_service.py#L177-L180)

### 数据模型与关系映射
- **角色与权限**
  - 多对多关系：Role.permissions
- **用户与角色**
  - 多对多关系：User.user_roles ↔ Role.user_roles
- **约束与索引**
  - 角色与权限代码唯一
  - 关键字段建立索引（如 code、resource）

```mermaid
erDiagram
PERMISSION {
uuid id PK
string code UK
string name
string resource
string action
boolean is_active
}
ROLE {
uuid id PK
string code UK
string name
boolean is_system
boolean is_active
}
USER_ROLE {
uuid id PK
uuid user_id FK
uuid role_id FK
}
ROLE ||--o{ USER_ROLE : "拥有"
PERMISSION ||--o{ ROLE : "被角色拥有"
```

**图表来源**
- [rbac_models.py:13-114](file://src/infrastructure/persistence/models/rbac_models.py#L13-L114)

**章节来源**
- [rbac_models.py:13-148](file://src/infrastructure/persistence/models/rbac_models.py#L13-L148)

### 权限模型与实体
- **PermissionEntity**
  - 字段：permission_id、name、code、resource、action、is_active
  - 行为：自动从 code 解析 resource 与 action，支持激活/停用
- **RoleEntity**
  - 字段：role_id、name、code、permissions、is_system、is_active
  - 行为：增删权限、激活/停用、序列化为字典

**章节来源**
- [permission_entity.py:11-85](file://src/domain/rbac/entities/permission_entity.py#L11-L85)
- [role_entity.py:11-80](file://src/domain/rbac/entities/role_entity.py#L11-L80)

### DTO 定义
- **RoleCreateDTO**：创建角色时的输入校验与示例
- **RoleUpdateDTO**：更新角色时的输入校验
- **AssignRoleDTO**：分配角色时的输入校验
- **UserRolesResponseDTO**：用户角色与权限列表的输出结构
- **RoleResponseDTO**：角色信息的标准响应格式
- **PermissionResponseDTO**：权限信息的标准响应格式

**章节来源**
- [role_create_dto.py:9-25](file://src/application/dto/rbac/role_create_dto.py#L9-L25)
- [role_update_dto.py:9-28](file://src/application/dto/rbac/role_update_dto.py#L9-L28)
- [assign_role_dto.py:9-21](file://src/application/dto/rbac/assign_role_dto.py#L9-L21)
- [user_roles_response_dto.py:11-17](file://src/application/dto/rbac/user_roles_response_dto.py#L11-L17)
- [role_response_dto.py:11-26](file://src/application/dto/rbac/role_response_dto.py#L11-L26)
- [permission_response_dto.py:11-25](file://src/application/dto/rbac/permission_response_dto.py#L11-L25)

### 统一响应格式
- **MessageResponse**：用于返回简单的操作结果消息
- **RoleListResponse**：角色列表的分页响应格式
- **PermissionListResponse**：权限列表的分页响应格式

**章节来源**
- [responses.py:13-20](file://src/api/common/responses.py#L13-L20)
- [rbac_controller.py:17-28](file://src/api/v1/controllers/rbac_controller.py#L17-L28)

## 依赖分析
- **组件耦合**
  - 控制器层依赖应用服务层；应用服务层依赖仓储层；仓储层依赖数据模型层；应用服务层依赖缓存管理器
- **外部依赖**
  - Django ORM、Django 缓存框架、Ninja/NinjaExtra 框架
- **循环依赖**
  - 通过分层与接口隔离避免循环依赖

```mermaid
graph LR
CTRL["rbac_controller.py"] --> SVC["rbac_service.py"]
SVC --> REPO["rbac_repo_impl.py"]
REPO --> MODELS["rbac_models.py"]
SVC --> CACHE["cache_manager.py"]
```

**图表来源**
- [rbac_controller.py:13](file://src/api/v1/controllers/rbac_controller.py#L13)
- [rbac_service.py:8](file://src/application/services/rbac_service.py#L8)
- [rbac_repo_impl.py:11](file://src/infrastructure/repositories/rbac_repo_impl.py#L11)
- [rbac_models.py:10](file://src/infrastructure/persistence/models/rbac_models.py#L10)
- [cache_manager.py:9](file://src/infrastructure/cache/cache_manager.py#L9)

**章节来源**
- [rbac_controller.py:1-259](file://src/api/v1/controllers/rbac_controller.py#L1-L259)
- [rbac_service.py:1-280](file://src/application/services/rbac_service.py#L1-L280)
- [rbac_repo_impl.py:1-251](file://src/infrastructure/repositories/rbac_repo_impl.py#L1-L251)
- [rbac_models.py:1-148](file://src/infrastructure/persistence/models/rbac_models.py#L1-L148)
- [cache_manager.py:1-149](file://src/infrastructure/cache/cache_manager.py#L1-L149)

## 性能考量
- **缓存命中率**
  - 权限检查优先从缓存读取，减少数据库查询压力
- **批量权限计算**
  - 通过角色集合合并去重，降低重复权限查询成本
- **索引优化**
  - 对权限 code、resource 等关键字段建立索引，提升查询效率
- **异步操作**
  - 使用 async ORM 方法，提高并发场景下的吞吐量
- **控制器优化**
  - 使用NinjaExtra的@api_controller装饰器，提供更好的性能和错误处理

## 故障排查指南
- **常见错误与处理**
  - 角色/权限不存在：返回相应的ValueError异常
  - 系统角色不可修改/删除：提示不可操作
  - 用户已拥有角色：避免重复分配
  - 用户无此角色：移除失败时抛出ValueError异常
- **测试覆盖**
  - 角色 CRUD、权限列表、权限初始化、用户角色分配/移除、用户权限检查等均有测试用例覆盖

**章节来源**
- [rbac_service.py:64-65](file://src/application/services/rbac_service.py#L64-L65)
- [rbac_service.py:88-89](file://src/application/services/rbac_service.py#L88-L89)
- [rbac_service.py:147-161](file://src/application/services/rbac_service.py#L147-L161)
- [rbac_controller.py:90](file://src/api/v1/controllers/rbac_controller.py#L90)
- [rbac_controller.py:224](file://src/api/v1/controllers/rbac_controller.py#L224)
- [test_rbac_api.py:23-174](file://tests/test_api/test_rbac_api.py#L23-L174)

## 结论
本 RBAC 子系统通过清晰的分层架构与完善的缓存策略，实现了高效、可扩展的角色与权限管理能力。控制器架构提供了更好的代码组织和错误管理能力，接口设计遵循 REST 规范，权限检查支持异步与缓存，满足生产环境的高可用需求。建议在生产环境中结合安全中间件与细粒度权限注解，进一步强化访问控制与审计能力。

## 附录
- **SQL 示例**
  - 可参考 sql/rbac.sql 中的表结构与索引定义，用于数据库初始化与迁移

**章节来源**
- [rbac.sql:18-148](file://sql/rbac.sql#L18-L148)